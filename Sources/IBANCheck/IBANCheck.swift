...
final class IBANChecks {
    ...
    class func check(iban: String) -> Bool {
        ...
        return mod(bignum: strUnderCheck, div:97) == 1
    }
    ...
    class func mod(bignum: String, div: Int) -> Int {
        ...
        var res = 0
        for i in 0..<bignum.count {
            if let digit = bignum[bignum.index(bignum.startIndex, offsetBy: i)].wholeNumberValue {
                res = (res*10 + digit) % div
            }else {
                return -1 //we have something else than a digit
            }
        }
        return res
    }
