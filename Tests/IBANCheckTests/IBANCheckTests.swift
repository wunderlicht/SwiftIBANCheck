//
//  IBANCheckTests.swift
//  IBAN CheckTests
//
//  Created by Torsten Wunderlich on 31.01.17.
//
import XCTest
@testable import IBANCheck
...
// mod tests
    func testModBiNumZero() {
        let bignum = "0"
        let divident = 4
        let expected = 0
        XCTAssertEqual(IBANChecks.mod(bignum: bignum, div: divident), expected)
    }
    
    func testModBiNumZeros() {
        let bignum = "000000000"
        let divident = 4
        let expected = 0
        XCTAssertEqual(IBANChecks.mod(bignum: bignum, div: divident), expected)
    }
    
    func testModNormalOperation() {
        let bignum = "123456789012345678901234567890123"
        let divident = 10
        let expected = 3
        XCTAssertEqual(IBANChecks.mod(bignum: bignum, div: divident), expected)
    }
